...
def create_output_directories():
    ...
    directories = ["full_analysis", "network_visualizations"]
...
def run_full_analysis(input_dir, output_dir, visualize=True):
    ...
    try:
        subprocess.run(["python", "analyze_full_textbook.py", 
                       "--input-dir", input_dir, 
    ...
    if visualize:
        ...
        try:
            subprocess.run(["python", "visualize_ksao_network.py", 
                           "--analysis-file", analysis_file], check=True)
        except subprocess.CalledProcessError as e:
    ...
    if visualize:
        print(f"Network visualizations saved to: network_visualizations/")
    
    return True

def main():
    parser = argparse.ArgumentParser(description="Run the complete KSAO analysis process")
    parser.add_argument("--input-dir", default="gemini_text_output", 
                       help="Directory containing chapter text files")
    parser.add_argument("--output-dir", default="full_analysis", 
